...
If MQTT client(s) OpenShift are on the same LAN, the a _LoadBalancer_ implementation such as MetalLB can be used.
...
. Review/modift configuration of the Artemis broker to be deployed in OpenShift:
...
-----
+
If not intending to use SNI enabled TLS for remote access, comment this entire section out.
...
+
-----
$ kubectl create secret generic artemis-sni-secret \
...
. Test using _mosquitto_pub_ client:
...
$ keytool -storetype pkcs12 -keystore server-ca-keystore.p12 -storepass securepass -alias server-ca -exportcert -rfc > $A_CERTS_DIR/server-ca.crt
$ openssl x509 -in $CERTS_DIR/server-ca.crt -text
-----

.. Invoke the remote Artemis broker using an mqtt client:
+
-----
$ export ARTEMIS_MQTT_HOST=$(oc get route artemis-mqtt-sni-0-svc-rte -n user1-flying-djl --template='{{ .spec.host }}')
